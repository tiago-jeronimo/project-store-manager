...
describe(' CAMADA CONTROLLER - Testa camada Service para requição de produtos', () => {
  ...
  describe('2 - Ao fazer requisição para método POST em "/products"', () => {

    describe('✅ Se for resolvido', () => {
      const response = {};
      const request = {};

      const products = { id: 4, name: 'Robocop' };

      before(() => {
        request.body = {};
        response.status = sinon.stub().returns(response);
        response.json = sinon.stub().returns();
        sinon.stub(SERVICE, 'postProduct').resolves(products)
      })

      after(() => {
        SERVICE.postProduct.restore();
      })
      ...
      it('2.3 - Se para os values das keys "id" e "name" são "4" e "Robocop", respectivamente ', async () => {
        await CONTROLLER.getProducts(request, response);
        expect(response.json.calledWith(products)).to.be.equal(true);
      });
    })
